...
trade_DF_Schock = np.zeros(len(t_res))
trade_CF_Schock = np.zeros(len(t_res))
trade_BE_Schock = np.zeros(len(t_res))
...
for i in range(len(t_res)):
    ...
    trade_DF_Schock[i] = abs(strom_spez[3,5])
    trade_CF_Schock[i] = abs(strom_spez[2,5])
    trade_BE_Schock[i] = abs(strom_spez[1,4])
...
trade_DF_base = np.abs(baseline_Handlesstrom_Daten[:, 3, 5])
trade_CF_base = np.abs(baseline_Handlesstrom_Daten[:, 2, 5])
trade_AC_base = np.abs(baseline_Handlesstrom_Daten[:, 0, 2])
trade_BE_base = np.abs(baseline_Handlesstrom_Daten[:, 1, 4])

trade_BF_base = np.abs(baseline_Handlesstrom_Daten[:, 1, 5])

plt.figure(figsize=(12,4))
plt.plot(t_res, (trade_DF_base - trade_DF_Schock), label="Δ trade_DF (Schock - Baseline)")
plt.plot(t_res, (trade_CF_base - trade_CF_Schock), label="Δ trade_CF (Schock - Baseline)")
plt.plot(t_res, (trade_AC_base - trade_AC_Schock), label="Δ trade_AC (Schock - Baseline)")
plt.plot(t_res, (trade_BE_base - trade_BE_Schock), label="Δ trade_BE (schwächste Route) (Schock - Baseline)")
plt.axvline(x=shock_start, color='red', linestyle='-.', label='Schock')
plt.axvline(x=shock_end, color='red', linestyle='-.', label='Schock')
plt.xlabel("Zeit")
